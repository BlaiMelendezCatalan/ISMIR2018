...
Util.write_audacity_labels(audacity_labels, "audio_1_combined.txt")

print " - Concatenating wav files..."
for i, fname in tqdm(enumerate(files_to_concatenate)):
    if i == 0:
        subprocess.check_output(["cp", fname, "audio_1_combined.wav"])
        continue
    command = ["sox", "audio_1_combined.wav", fname, "temp.wav"]
    subprocess.check_output(command)
    subprocess.check_output(["cp", "temp.wav", "audio_1_combined.wav"])
